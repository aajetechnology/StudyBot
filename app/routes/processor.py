...
def process_log(filename):
    ...
    def generate():
        ...
        if ext in ['pdf', 'docx', 'png', 'jpg', 'jpeg', 'webp']:
            yield f"data: {json.dumps({'msg': '>>> DETECTED DOCUMENT FORMAT. INITIALIZING AI OCR...', 'class': 'text-warning'})}\n\n"
        ...
        if not transcript_text or len(transcript_text.strip()) < 10:
            yield f"data: {json.dumps({'msg': '❌ ERROR: Could not extract sufficient text from this file.', 'class': 'text-danger'})}\n\n"
            return
